...
#TODO:
    # fix the few cases where electicity socket screws up image. - 5,6,7,8,11,20,29
...
def real_coordinates_circle_detection(line_detection_score_arr,image_to_real_coordinate_transform,real_to_image_coordinate_transform,b_draw = False,save_figures_path=None):
    image_to_real_coordinate_transform = lambda x:x
    real_to_image_coordinate_transform = lambda x: x
    xleft,yleft = np.meshgrid(range(line_detection_score_arr.shape[0]),range(line_detection_score_arr.shape[1]),indexing="ij")
    # find initial guess
    (x0_e1, y0_e1, R_e1),(x0_e2, y0_e2, R_e2),x_intersection = make_initial_guesses_for_circles(image_to_real_coordinate_transform,line_detection_score_arr,xleft,yleft)
...
def make_initial_guesses_for_circles(image_to_real_coordinate_transform,line_detection_score_arr,x,y):
...
out_file_name = r"..\analyse_data\resaults_analisys_v2_temp"
b_clean_run = True
b_DRAW = False
...
for fn in pbar(sorted(os.listdir()),position=1):
    ...
    real_to_image_coordinate_transform,image_to_real_coordinate_transform = get_image_and_real_coordinates_converters(points_im,origin_of_image_points,image.shape)
