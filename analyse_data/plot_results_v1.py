...
key_photo_index = 0; key_part_soap = 1; key_tension = 2; key_is_garbage = 3; key_R1 = 4; key_a1 = 5; key_R2 = 6; key_a2 = 7; key_R_std = 8
number_of_fields = 9
...
def extract_as_data_points(data_original, relable_istrash):
    ...
    data_points2[:, key_a1] = data_original[:, key_a2]
    data_points1[:, key_R2] = np.nan
    data_points1[:, key_a2] = np.nan
    data_points2[:, key_R2] = np.nan
    data_points2[:, key_a2] = np.nan
...
data_original = load_from_file(r"resaults_analisys_v1")
...
for part_soap in np.unique(data_points[:,key_part_soap]):
    ...
    a = plt.errorbar(
        data_points[idx_soup * (~idx_garbage), key_tension], data_points[idx_soup * (~idx_garbage), key_R1],data_points[idx_soup * (~idx_garbage), key_R_std]*2,
        fmt="o",capsize=3,label=f"part_soap={part_soap*100:.3g}%",alpha=0.3)

    colors.append(a.get_children()[0].get_color())

    plt.plot(data_points[idx_soup * (idx_garbage), key_tension], data_points[idx_soup * (idx_garbage), key_R1], "xr",alpha=0.2)

    # linear fit
    num_of_points = sum(idx_soup * (~idx_garbage))
    (ST,_),((var_ST,_),(_,_)) = np.polyfit([0] + list(data_points[idx_soup * (~idx_garbage), key_R1]),
               [0] + list(data_points[idx_soup * (~idx_garbage), key_tension]),
               deg=1,
