...
data_points = load_from_file(r"temp")
...
for part_soap in np.unique(data_points[:,key_part_soap]):
    ...
    plt.plot([0,2000],[0,2000/ST],label=f"part_soap={part_soap*100:.3g}: a = {1/ST:3g}",color=colors[-1])
...
for i in range(len(measured_surface_tensions)):
    plt.errorbar(measured_surface_tensions[i,0]*100,measured_surface_tensions[i,1],measured_surface_tensions[i,2]*2,fmt="none",capsize=3,ecolor=colors[i],label=f"measured for {measured_surface_tensions[i,0]*100:3g}% echo dish soap")
