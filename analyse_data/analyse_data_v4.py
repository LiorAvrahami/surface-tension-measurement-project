...
def real_coordinates_circle_detection(line_detection_score_arr,image_to_real_coordinate_transform,real_to_image_coordinate_transform,b_draw = False,save_figures_path=None):
    ...
    def fit_circle(points_x,points_y,weights,x0e,y0e,Re,is_left_side):
        ...
        if b_draw:
            ...
            plt.savefig(os.path.join(r"..\analyse_data\image_results_v3", save_figures_path + add_to_name), dpi=200)
            plt.close()
...
out_file_name = r"..\analyse_data\resaults_analisys_v3_temp"
b_clean_run = False
b_DRAW = True
...
for fn in pbar(sorted(os.listdir()),position=1):
    ...
    if b_DRAW:
        ...
        plt.savefig(os.path.join(r"..\analyse_data\image_results_v3",fn),dpi=200)
        plt.close()
